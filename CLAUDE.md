...
TA 포트폴리오 기반으로 통합 중인 사이트. GitHub Pages + 커스텀 도메인 `tommlimm.net`.

기본 브랜치는 `gh-pages`이며, 통합 작업은 `codex/ta-portfolio-migration`에서 진행한다. 가져온 TA 원본 커밋, 기존 포폴 원본과 이식 목록은 [docs/portfolio-migration.md](docs/portfolio-migration.md)를 참고한다. 기존 포폴 작업은 새 템플릿에 하나씩 이식하고 공개 목록에 명시한다.
...
- 가져온 검증 워크플로는 `gh-pages`와 `codex/**` 푸시에서 실행한다. 릴리즈 워크플로는 `gh-pages` 이력에 포함된 `vMAJOR.MINOR.PATCH` 태그로 검증 → Pages 배포 → GitHub Release 생성을 실행한다.
- 저장소의 Pages 설정은 이번 이식에서 변경하지 않았다. 운영 브랜치 병합·푸시 전에 배포 요청을 확인하고, `site-public.json`을 우회하는 브랜치 직접 배포가 아닌 `_site/` 아티팩트 기반 GitHub Actions 배포로 설정해야 한다.
